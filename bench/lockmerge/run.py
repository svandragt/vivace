    ...
    with tempfile.TemporaryDirectory(prefix="lockmerge-") as tmp:
        ...
        for m in merges:
            ...
            if base_idx is None or ours_idx is None or theirs_idx is None:
                ...
                continue
            real = real_conflicts(base_idx, ours_idx, theirs_idx)
...
def self_test() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="lockmerge-selftest-") as tmp:
        ...
        # both sides change a/a to different versions -> a real conflict.
        conflict_pkgs = [
            {"name": "a/a", "version": "1.2.0", "source": {"reference": "aa2"}},
            {"name": "b/b", "version": "1.0.0", "source": {"reference": "bbb"}},
        ]
        commit("ours2 bumps a further", conflict_pkgs)
        ...
        # a real merge conflict is expected on composer.lock; resolve trivially.
        if (repo / "composer.lock").exists():
            git(repo, "checkout", "--quiet", "--ours", "composer.lock")
            git(repo, "add", "composer.lock")
        ...
        real = real_conflicts(base_idx, ours_idx, theirs_idx)
        assert real == 1, f"expected exactly one real conflict (a/a), got {real}: {base_idx} {ours_idx} {theirs_idx}"
