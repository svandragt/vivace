...
- `package` repositories: packages declared inline in `composer.json` rather than fetched from a registry ([#294](https://github.com/svandragt/vivace/issues/294))
- `viv lock merge` re-solves what diverged in three rungs, the divergent closure, then pinned packages that directly require it, then a full solve that prefers every locked version; `--max-scope` caps it and the report names the rung and every package that moved ([#296](https://github.com/svandragt/vivace/issues/296)). `viv.lock` inputs re-solve the same way, taking the pinned set's requires from the sibling `composer.lock` ([#295](https://github.com/svandragt/vivace/issues/295))
- `viv install` reads `viv.lock` beside `composer.lock` when present and refuses if the two disagree in either direction; `viv update` implies `--lock native` once `viv.lock` exists ([#297](https://github.com/svandragt/vivace/issues/297))
- `viv install` on a lock left with git conflict markers names the file, the first marker's line and the packages in conflict ([#274](https://github.com/svandragt/vivace/issues/274)); inside a git checkout mid-merge it resolves the conflict itself from the index stages, the same way the merge driver would, and continues ([#299](https://github.com/svandragt/vivace/issues/299))
- `viv init` writes `composer.lock merge=viv` and `viv.lock merge=viv` to `.gitattributes`; `viv install` sets `merge.viv.driver` in any clone whose `.gitattributes` names it, so git merges locks through viv without a manual step ([#298](https://github.com/svandragt/vivace/issues/298))
