...
[^20]: [#316](https://github.com/svandragt/vivace/issues/316): craftcms/craft's `yii2-shell` requirement is satisfiable by both `2.0.6` and `dev-master`; which one wins depends on whether `dev-master` is blocked by a security advisory at solve time. viv picks `2.0.6`, Composer picked `dev-master` on the day this sweep ran. `v0.16.0`'s sweep against the same frozen mirror resolves `2.0.6` too, and the pinned corpus's install row (from a lock Composer wrote) is identical either way.
