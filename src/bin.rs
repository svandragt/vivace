...
/// proxy, or a symlink into `vendor/`), never a file we don't recognise.
///
/// ponytail: the `sh` proxy carries no such marker (Composer's own doesn't
/// either), so a stale `sh`-style proxy for a removed package is not caught
/// here; add a marker comment if that turns out to matter in practice.
    ...
    for entry in fs_err::read_dir(bin_dir)? {
        ...
        let looks_generated = !is_symlink_into_vendor
            && fs_err::read_to_string(&path)
                .is_ok_and(|content| content.contains("generated by Composer"));
