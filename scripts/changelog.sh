...
section=$(echo "$issues_json" | jq -r '
	def bucket:
		...
		| if ($labels | index("bug")) or (.title | test("^Fix "; "i")) then "Fixed"
		elif $labels | index("performance") then "Performance"
		elif $labels | index("tooling") then "Tooling"
		elif (.title | test("should|no longer|instead"; "i")) then "Changed"
		else "Added"
		end;
	["Added", "Changed", "Fixed", "Performance", "Tooling"] as $order
	| map(. + {bucket: bucket})
	| group_by(.bucket)
...
EOF
	...
	link_line="[$version]: https://github.com/$REPO/releases/tag/v$version"
