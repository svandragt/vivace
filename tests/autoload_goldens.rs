...
/// #259: `hoa/consistency`, `hoa/event` and `hoa/exception` require one
/// another in a cycle; `symfony/polyfill-mbstring` sits outside it but is
/// itself required by three unrelated packages, giving it a weight
/// (`PackageSorter`'s cycle-breaking depends on visit order, not only on the
/// final weight) that lands it ahead of the cycle in Composer's own output.
/// Expected order and md5 keys captured from a real `composer install`
/// (Composer 2.10.2) on this exact package graph, a devbox scratch project.
fn files_autoload_order_survives_a_dependency_cycle_case() -> Case {
    Case {
        root_requires: &[
            "hoa/consistency",
        ...
        packages: vec![
            Pkg::new("hoa/consistency", json!({"files": ["Prelude.php"]}))
                .requires(&["hoa/event", "hoa/exception"]),
            Pkg::new("hoa/event", Value::Null).requires(&["hoa/consistency", "hoa/exception"]),
            Pkg::new("hoa/exception", Value::Null).requires(&["hoa/consistency", "hoa/event"]),
            Pkg::new(
                "symfony/polyfill-mbstring",
                json!({"files": ["bootstrap.php"]}),
            ),
            Pkg::new("vendora/leafa", Value::Null).requires(&["symfony/polyfill-mbstring"]),
            Pkg::new("vendorb/leafb", Value::Null).requires(&["symfony/polyfill-mbstring"]),
            Pkg::new("vendorc/leafc", Value::Null).requires(&["symfony/polyfill-mbstring"]),
        ],
        files: &[
            (
                "composer-test-autoload/hoa/consistency/Prelude.php",
                "<?php // hoa/consistency prelude",
            ),
            (
                "composer-test-autoload/symfony/polyfill-mbstring/bootstrap.php",
...
/// drupal/recommended-project 11.4.6's `pear/*` chain (`archive_tar` ->
/// `pear-core-minimal` -> `{console_getopt, pear_exception}`): Composer's
/// `include_paths.php` follows install order (dependencies-first DFS, a
/// sibling's *last*-declared `require` visited first), not the packages'
/// declaration order — regressed against the v0.6.0 compat sweep.
fn include_paths_follow_install_order_case() -> Case {
    Case {
        // Declared alphabetically, the order a lock file (and a naive
        // `Input`) lists them in — not the install order the golden expects,
        // so this only passes once the packages get reordered.
        packages: vec![
            Pkg::new("pear/archive_tar", Value::Null)
                .requires(&["pear/pear-core-minimal"])
                .include_path(&["./"]),
            Pkg::new("pear/console_getopt", Value::Null).include_path(&["./"]),
            Pkg::new("pear/pear-core-minimal", Value::Null)
                .requires(&["pear/console_getopt", "pear/pear_exception"])
                .include_path(&["src/"]),
            Pkg::new("pear/pear_exception", Value::Null).include_path(&["."]),
        ],
        suffix: "_13",
        inline: &[(
            INCLUDE_PATHS,
...
cases! {
    ...
    files_autoload_order_survives_a_dependency_cycle => files_autoload_order_survives_a_dependency_cycle_case,
    ...
    include_paths_follow_install_order => include_paths_follow_install_order_case,
